...
import { DeviceType } from '@/types/util';
...
export default class extends Vue {
  ...
  protected get deviceIsMobile() {
    return this.appModule.deviceType === DeviceType.Mobile;
  }
  ...
  mounted() {
    ...
    if (isMobile) {
      this.appModule.setDeviceType(DeviceType.Mobile);
      this.appModule.setSidebar(false);
    }
  }
  ...
  private resizeHandler() {
    if (!document.hidden) {
      const isMobile = this.isMobile();
      this.appModule.setDeviceType((isMobile ? DeviceType.Mobile : DeviceType.Desktop));
      if (isMobile) {
